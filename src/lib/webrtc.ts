...
export function describeMediaError(error: unknown): string {
  ...
  if (error.name === "NotAllowedError") {
    return "Camera, microphone, or screen permission was blocked. Allow it in your browser settings and try again.";
  }
  if (error.name === "NotFoundError") {
    return "Flux could not find the selected camera or microphone.";
  }
  if (error.name === "NotReadableError") {
    return "Another app may already be using this camera or microphone.";
  }
  if (error.name === "OverconstrainedError") {
    return "The selected camera cannot provide the requested quality. Choose another device.";
  }
...
export async function captureFluxMedia(options: CaptureOptions): Promise<CapturedMedia> {
  if (!mediaCaptureReady()) {
    throw new Error("Camera and microphone require a secure HTTPS page or the Flux app.");
  }
...
async function captureScreen(options: CaptureOptions): Promise<CapturedMedia> {
  ...
  const allSourceStreams = [displayStream, microphoneStream].filter(Boolean) as MediaStream[];
  ...
  const cleanup = () => {
    allSourceStreams.forEach((source) => source.getTracks().forEach((track) => track.stop()));
