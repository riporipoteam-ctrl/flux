...
export function RecRoomFeatureBanner() {
  ...
  const vm = status?.vmRuntime;
  const vmSupported = Boolean(vm?.supported);
  const vmReady = Boolean(vm?.readyForGame);
  const sessions = Number(status?.sessions || 0);
  const maxVms = Number(vm?.maxVms || 0);
  const detail = status?.error || vm?.reason || vm?.warning || "Disposable Windows VM pool ready.";
      ...
      <div className="relative overflow-hidden rounded-[28px] border border-border bg-[#07111f] text-white shadow-sm">
        ...
        <div className="relative p-6 sm:p-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-3xl">
              <div className="flex flex-wrap gap-2">
                <Pill icon={Cpu}>RipoTeamServer VM</Pill>
                <Pill icon={Cloud}>Browser game</Pill>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-6 text-white/62 sm:text-base">
                Press Play and RipoTeamServer creates a disposable Windows VM for you, boots the exact May 19, 2022 client, loads your Flux-backed identity and streams the running game into Flux. Leaving destroys the temporary VM while supported account and save state remain persistent.
              </p>
              <p className="mt-3 text-xs font-semibold text-white/42">Build 8751857 · Depot 471711 · Manifest 6337851004861751095 · copy-on-write per-player disk</p>
            </div>

            <button type="button" onClick={() => void refreshStatus()} className="inline-flex h-10 shrink-0 items-center justify-center gap-2 self-start rounded-full border border-white/12 bg-white/7 px-4 text-xs font-black text-white/75 transition hover:bg-white/12" aria-label="Refresh Rec Room VM status">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <LiveStat icon={Cpu} label="Windows VM runtime" value={loading && !status ? "Checking…" : vmReady ? "Game-ready" : vmSupported ? "KVM ready · GPU pending" : "Unavailable"} detail={detail} good={vmReady} />
            <LiveStat icon={Users} label="Disposable sessions" value={`${sessions}${maxVms ? ` / ${maxVms}` : ""} active`} detail="One temporary Windows VM per player" good={sessions > 0} />
          </div>

          <div className={`mt-4 rounded-2xl border p-4 ${vmReady ? "border-emerald-300/15 bg-emerald-300/[.07]" : "border-amber-300/15 bg-amber-300/[.07]"}`}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className={`text-sm font-black ${vmReady ? "text-emerald-100" : "text-amber-100"}`}>
                  {vmReady ? "RipoTeamServer can create a Windows game VM on demand." : "The Flux VM workflow is wired; this server runtime still has a virtualization blocker."}
                </p>
                <p className={`mt-1 max-w-3xl text-xs leading-5 ${vmReady ? "text-emerald-50/55" : "text-amber-50/55"}`}>
                  {vmReady ? "Open Rec Room, press Play, and the loading screen will track Windows VM creation until the live game replaces it." : detail}
                </p>
              </div>
