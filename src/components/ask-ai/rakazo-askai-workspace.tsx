...
export default function RakazoAskAIWorkspace() {
  ...
  useEffect(() => {
    ...
    if (activeConversationId.startsWith("local-")) {
      setMessages(readLocalMessages(user.uid, activeConversationId));
      return;
    }
    getMessages(activeConversationId)
      .then((items) => setMessages(items.length ? items : readLocalMessages(user.uid, activeConversationId)))
      .catch(() => setMessages(readLocalMessages(user.uid, activeConversationId)));
  ...
  const appendMessage = useCallback(async (conversationId: string, role: "user" | "assistant", content: string, meta?: Record<string, unknown>) => {
    ...
    saveLocalMessage(user.uid, conversationId, message);
  ...
  const ensureConversation = useCallback(async (title: string) => {
    if (!user) throw new Error("Sign in to use the AskAI workspace.");
    ...
    try {
      ...
      saveLocalConversation(user.uid, { id, userId: user.uid, title: title.slice(0, 80) || "New thread", createdAt: Date.now(), updatedAt: null });
      setActiveConversationId(id);
      return id;
    }
  }, [activeConversationId, user]);
  ...
  const sendMessage = useCallback(async (rawText: string) => {
    ...
    if (!text || busy) return;
    if (!user) {
      setError("Sign in to use your Flux workspace.");
      return;
    }
    setBusy(true);
    setError(null);
    ...
    saveLocalMessage(user.uid, conversationId, userMessage);
    if (!conversationId.startsWith("local-")) await addMessage(conversationId, { role: "user", content: text }).catch(() => undefined);
  ...
  const deleteThread = async (conversation: AIConversation) => {
    ...
    else await deleteConversation(conversation.id).catch(() => undefined);
    ...
    <div className="rakazo-shell" data-testid="rakazo-askai-workspace">
      ...
      <aside className={`rakazo-sidebar ${leftOpen ? "is-open" : ""}`}>
        ...
        <div className="rakazo-account-wrap">
          {accountOpen ? <div className="rakazo-account-menu"><button type="button" onClick={() => toggleRight("settings")}><Settings2 size={15} /> Workspace settings</button><button type="button" onClick={() => void signOut()}><Archive size={15} /> Sign out</button></div> : null}
          <button type="button" className="rakazo-account" onClick={() => setAccountOpen((value) => !value)}><span className="rakazo-account-avatar">{initials}</span><span><strong>{displayName}</strong><small>{user?.email || "Flux account"}</small></span><ChevronDown size={15} /></button>
        </div>
      </aside>
