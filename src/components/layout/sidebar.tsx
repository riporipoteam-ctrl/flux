...
const items = [
  ...
  { href: "/games", label: "Games", icon: Gamepad2, featured: true },
    ...
    <aside className="sticky top-0 hidden h-[100dvh] w-[82px] shrink-0 flex-col border-r border-border/70 bg-sidebar px-2.5 py-4 backdrop-blur-2xl lg:flex xl:w-[286px] xl:px-4">
      ...
      <nav className="no-scrollbar flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto py-1">
        {items.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cn("nav-item group relative flex min-h-11 items-center gap-3 overflow-hidden rounded-[17px] px-3 py-2.5 text-[14px] font-bold", active ? "text-foreground" : "text-muted-foreground hover:text-foreground", item.featured && !active && "text-emerald-500")}>
              {active ? <motion.span layoutId="nav-pill" className="absolute inset-0 rounded-[17px] border border-primary/15 bg-gradient-to-r from-primary/18 via-primary/10 to-cyan-400/8" transition={{ type: "spring", stiffness: 390, damping: 34 }} /> : <span className="absolute inset-0 rounded-[17px] opacity-0 transition duration-200 group-hover:bg-muted/70 group-hover:opacity-100" />}
              <span className={cn("relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-xl transition", active && "bg-primary text-white shadow-[0_8px_24px_rgba(91,77,245,.28)]", item.featured && !active && "bg-emerald-500/10")}><Icon className="h-[18px] w-[18px]" strokeWidth={active ? 2.5 : 2.1} /></span>
              <span className="relative z-10 hidden flex-1 xl:inline">{item.label}</span>
              {item.href === "/notifications" && unread > 0 ? <span className="relative z-10 ml-auto hidden h-5 min-w-5 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-cyan-400 px-1.5 text-[10px] font-black text-white xl:flex">{unread > 9 ? "9+" : unread}</span> : item.featured ? <span className="relative z-10 ml-auto hidden rounded-full bg-emerald-500/10 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-600 xl:inline">Farm</span> : null}
            </Link>
          );
        })}

        <Link href={profileHref} className={cn("nav-item group relative flex min-h-11 items-center gap-3 rounded-[17px] px-3 py-2.5 text-[14px] font-bold", pathname.startsWith("/profile") ? "text-foreground" : "text-muted-foreground hover:text-foreground")}>
      ...
      <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
        <DialogTrigger asChild><Button variant="flux" className="mt-3 w-full rounded-[18px]" size="lg"><PenSquare className="h-4 w-4" /><span className="hidden xl:inline">Create post</span></Button></DialogTrigger>
        <DialogContent className="max-w-xl overflow-hidden p-0 sm:rounded-[30px]"><DialogHeader className="border-b border-border px-5 py-4"><DialogTitle>Create post</DialogTitle></DialogHeader><div className="p-4"><ComposeBox onSuccess={() => setComposeOpen(false)} autofocus /></div></DialogContent>
      </Dialog>
