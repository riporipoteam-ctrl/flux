    ...
    <aside className="sticky top-0 hidden h-[100dvh] w-[76px] shrink-0 flex-col border-r border-border bg-sidebar px-2 py-3 lg:flex xl:w-[250px] xl:px-3">
      ...
      <nav className="no-scrollbar flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto py-1">
        {primaryItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cn("group relative flex min-h-[50px] items-center gap-4 rounded-2xl px-3 text-[15px] font-semibold transition-colors xl:text-[17px]", active ? "bg-foreground text-background" : "text-foreground hover:bg-muted")}>
        ...
        <Link href={profileHref} className={cn("flex min-h-[50px] items-center gap-4 rounded-2xl px-3 text-[15px] font-semibold transition-colors xl:text-[17px]", pathname.startsWith("/profile") ? "bg-foreground text-background" : "hover:bg-muted")}>
          <span className="grid h-8 w-8 place-items-center"><User className="h-[22px] w-[22px]" /></span>
          <span className="hidden xl:inline">Profile</span>
        </Link>

        <DropdownMenu>
          <DropdownMenuTrigger asChild><button type="button" className="flex min-h-[50px] items-center gap-4 rounded-2xl px-3 text-[15px] font-semibold transition-colors hover:bg-muted xl:text-[17px]"><span className="grid h-8 w-8 place-items-center"><MoreHorizontal className="h-[22px] w-[22px]" /></span><span className="hidden xl:inline">More</span></button></DropdownMenuTrigger>
          <DropdownMenuContent side="right" align="end" className="max-h-[min(680px,calc(100dvh-24px))] w-[286px] overflow-y-auto rounded-2xl border-border p-2 shadow-[0_20px_70px_rgba(0,0,0,.18)]">
            {moreSections.map((section, sectionIndex) => <div key={section.label}>{sectionIndex ? <DropdownMenuSeparator /> : null}<DropdownMenuLabel className="px-3 py-2 text-[10px] font-black uppercase tracking-[.14em] text-muted-foreground">{section.label}</DropdownMenuLabel>{section.items.map(({ href, label, icon: Icon }) => <DropdownMenuItem key={href} asChild className="rounded-xl py-2.5 font-semibold"><Link href={href} className="flex items-center gap-3"><Icon className="h-5 w-5" />{label}</Link></DropdownMenuItem>)}</div>)}
            {profile?.isAdmin ? <><DropdownMenuSeparator /><DropdownMenuItem asChild className="rounded-xl py-2.5 font-semibold"><Link href="/admin" className="flex items-center gap-3"><Shield className="h-5 w-5" />Admin command center</Link></DropdownMenuItem></> : null}
          </DropdownMenuContent>
        </DropdownMenu>
      </nav>
