...
import { ChevronLeft, ChevronRight, Music2, Pause, Play, Share2, Volume2, VolumeX, X } from "lucide-react";
...
export function StoryViewer({ groups, initialGroup, onClose }: { groups: StoryGroup[]; initialGroup: number; onClose: () => void }) {
  ...
  useEffect(() => {
    if (story && user) void markStoryViewed(story.id, user.uid);
  }, [story, user]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483600] flex bg-black text-white">
      <button type="button" onClick={onClose} className="absolute right-3 top-[max(0.75rem,env(safe-area-inset-top))] z-30 grid h-11 w-11 place-items-center rounded-full bg-black/45 backdrop-blur-md" aria-label="Close Stories"><X className="h-5 w-5" /></button>
      ...
      <div className="relative mx-auto flex h-full w-full max-w-[520px] flex-col overflow-hidden bg-[#111] sm:my-4 sm:h-[calc(100%-2rem)] sm:rounded-2xl">
        <div className="absolute inset-x-0 top-0 z-20 bg-gradient-to-b from-black/75 to-transparent px-3 pb-12 pt-[max(0.7rem,env(safe-area-inset-top))]">
          <div className="flex gap-1">
            {group.stories.map((item, index) => <span key={item.id} className="h-0.5 flex-1 overflow-hidden rounded-full bg-white/35"><span className="block h-full bg-white transition-[width] duration-75" style={{ width: `${index < storyIndex ? 100 : index === storyIndex ? progress * 100 : 0}%` }} /></span>)}
          </div>
          <div className="mt-3 flex items-center gap-2 pr-12">
            <UserAvatar user={group.author} size="sm" clickable={false} />
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{group.author?.displayName || "Flux user"}</p><p className="truncate text-[11px] text-white/70">@{group.author?.username || "user"}</p></div>
            <button type="button" onClick={() => { onClose(); router.push(`/messages?story=${encodeURIComponent(story.id)}`); }} className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/15" aria-label="Share story to Messages"><Share2 className="h-4 w-4" /></button>
            <button type="button" onClick={() => setPaused((value) => !value)} className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/15" aria-label={paused ? "Play" : "Pause"}>{paused ? <Play className="h-4 w-4 fill-white" /> : <Pause className="h-4 w-4 fill-white" />}</button>
            <button type="button" onClick={() => setMuted((value) => !value)} className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/15" aria-label={muted ? "Unmute" : "Mute"}>{muted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}</button>
          </div>
        </div>

        <div className={cn("relative flex flex-1 items-center justify-center overflow-hidden", designClass(story.designId))}>
          {story.mediaType === "video" ? (
            <video ref={videoRef} src={assetUrl(story.mediaUrl)} className="h-full w-full object-contain" playsInline autoPlay muted={muted} onTimeUpdate={(event) => { const video = event.currentTarget; if (video.duration) setProgress(video.currentTime / video.duration); }} onEnded={goNext} />
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={assetUrl(story.mediaUrl)} alt="Story" className="h-full w-full object-contain" />
          )}

          {story.text ? <div className={cn("pointer-events-none absolute inset-x-5 flex", positionClass(story.textPosition))}><p className={cn("max-w-full whitespace-pre-wrap rounded-xl bg-black/25 px-4 py-2 text-center text-2xl leading-tight shadow-lg backdrop-blur-[2px]", textStyleClass(story.textStyle))} style={{ color: story.textColor }}>{story.text}</p></div> : null}
          {track ? <div className="absolute bottom-[max(1.5rem,env(safe-area-inset-bottom))] left-4 flex items-center gap-2 rounded-full bg-black/55 px-3 py-2 text-xs font-semibold backdrop-blur-md"><Music2 className="h-4 w-4" /><span>{track.title}</span></div> : null}
