...
export default function LiveStudioV4() {
  ...
  const attachPreview = async (capture = captureRef.current) => {
    ...
    if (videoRef.current.srcObject !== capture.stream) videoRef.current.srcObject = capture.stream;
    videoRef.current.muted = true;
    videoRef.current.playsInline = true;
    await videoRef.current.play().catch(() => undefined);
  };

  const openCamera = async (): Promise<CapturedMedia | null> => {
  ...
  const cleanupPeer = (viewerId: string) => {
    ...
    attemptsRef.current.delete(viewerId);
  ...
  const connectViewer = async (
    ...
    attemptsRef.current.set(viewerId, livePeer.attempt);
    ...
    try {
      ...
      peer.onicecandidateerror = (event) => {
        console.warn("Host ICE error", event.errorCode, event.errorText);
      };
      peer.onconnectionstatechange = () => {
        const connected = [...peersRef.current.values()]
          .filter((item) => item.connectionState === "connected").length;
      ...
      await setReliableLiveOffer(activeStreamId, viewerId, livePeer.attempt, {
        type: offer.type,
        sdp: offer.sdp,
      });
    } catch (error) {
  ...
  const reconcilePeers = (
    ...
    for (const item of items) {
      const existingAttempt = attemptsRef.current.get(item.viewerId);
      if (existingAttempt !== undefined && existingAttempt !== item.attempt) {
  ...
  const startLive = async () => {
    ...
    setStarting(true);
    try {
      const capture = captureRef.current || await openCamera();
      if (!capture?.stream.getTracks().some((track) => track.readyState === "live")) {
        throw new Error("Open the camera again before going live.");
      }

      const id = await createLiveStream({
        hostId: user.uid,
        title: title.trim(),
        description: "Live on Flux",
        category: "Chatting",
        sourceType: "camera",
      });
      setStreamId(id);
      ...
      globalStopsRef.current.push(
        subscribeReliableLivePeers(id, (items) => reconcilePeers(id, items, capture.stream))
      );
      await attachPreview(capture);
  ...
  const copyLink = async () => {
    ...
    await navigator.clipboard.writeText(
      `${window.location.origin}${base}/live/view/?id=${encodeURIComponent(streamId)}`
    );
      ...
      <div className="grid h-full lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="relative h-full overflow-hidden bg-black">
          ...
          <header className="absolute inset-x-0 top-0 z-20 flex items-center gap-2 px-3 pt-[max(.7rem,env(safe-area-inset-top))] sm:px-5">
            <span className={cn("flex items-center gap-2 rounded-full px-3 py-2 text-[10px] font-black", isLive ? "bg-red-500" : "bg-black/55")}>
              <span className="h-2 w-2 rounded-full bg-white" />{isLive ? "LIVE" : "PREVIEW"}
            </span>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-black">{title || "Flux Live Studio"}</p>
              <p className="truncate text-[10px] text-white/58">{status}</p>
            </div>
            {isLive ? (
              <button onClick={() => void copyLink()} className="grid h-10 w-10 place-items-center rounded-full bg-black/55" aria-label="Copy live link">
                <Copy className="h-4.5 w-4.5" />
              </button>
            ) : null}
            <button onClick={() => void stopLive()} className="grid h-10 w-10 place-items-center rounded-full bg-black/55" aria-label="End or close">
          ...
          {!previewReady ? (
            <div className="absolute inset-0 grid place-items-center p-6 text-center">
              <div>
                <span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-white/8"><Camera className="h-8 w-8 text-white/60" /></span>
                <h1 className="mt-4 text-2xl font-black">Prepare your camera</h1>
                <p className="mt-2 text-sm text-white/48">The preview stays attached to one video element so it cannot turn black when Live starts.</p>
                <button onClick={() => void openCamera()} disabled={preparing} className="mt-5 inline-flex h-12 items-center gap-2 rounded-full bg-white px-6 font-black text-black disabled:opacity-50">
                  {preparing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Video className="h-4 w-4" />}Open camera
                </button>
              </div>
            </div>
          ) : null}
          ...
          <div className="absolute inset-x-3 bottom-[max(1rem,env(safe-area-inset-bottom))] z-20 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2">
            {!isLive ? (
              <div className="w-full max-w-xl rounded-[22px] bg-black/72 p-3 backdrop-blur-xl">
                <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Name your live" className="h-11 w-full rounded-full border border-white/14 bg-white/8 px-4 text-sm font-semibold outline-none" />
                <div className="mt-3 grid grid-cols-[auto_auto_1fr] gap-2">
                  <button onClick={toggleMic} className="grid h-11 w-11 place-items-center rounded-full bg-white/10">{microphoneEnabled ? <Mic className="h-4.5 w-4.5" /> : <MicOff className="h-4.5 w-4.5" />}</button>
                  <button onClick={toggleCamera} className="grid h-11 w-11 place-items-center rounded-full bg-white/10">{cameraEnabled ? <Video className="h-4.5 w-4.5" /> : <VideoOff className="h-4.5 w-4.5" />}</button>
                  <button onClick={() => void startLive()} disabled={!title.trim() || starting || !previewReady} className="flex h-11 items-center justify-center gap-2 rounded-full bg-red-500 px-6 text-sm font-black disabled:opacity-40"><Radio className="h-4 w-4" />{starting ? "Starting…" : "Go Live"}</button>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 rounded-full bg-black/72 px-4 py-3 text-xs font-black backdrop-blur-xl">
                <Wifi className="h-4 w-4 text-emerald-400" />{connectedViewers} receiving video
                <span className="text-white/38">·</span><Eye className="h-4 w-4" />{analytics.activeViewers}
              </div>
            )}
          </div>
        </section>
        ...
        <aside className="hidden h-full min-h-0 border-l border-white/10 bg-[#090b0f] lg:flex lg:flex-col">
          <div className="border-b border-white/10 p-4">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="font-black">Host chat</h2><p className="text-[11px] text-white/42">Comments appear here in real time</p></div>
              <Settings2 className="ml-auto h-4 w-4 text-white/35" />
            </div>
          </div>
          ...
          <div className="border-t border-white/10 p-3">
            <div className="flex gap-2">
              <input value={comment} onChange={(event) => setComment(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void postComment(); }} placeholder="Write as host…" className="h-10 min-w-0 flex-1 rounded-full bg-white/8 px-4 text-sm outline-none" />
              <button onClick={() => void postComment()} className="grid h-10 w-10 place-items-center rounded-full bg-white text-black"><Send className="h-4 w-4" /></button>
            </div>
          </div>
        </aside>
      </div>
