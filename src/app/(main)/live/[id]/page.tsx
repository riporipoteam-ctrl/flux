...
export default function LiveViewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!user || stream?.status !== "live" || stream.hostId === user.uid || peerRef.current) return;
    let cancelled = false;
  ...
  if (!stream) return <div className="grid min-h-[70vh] place-items-center"><Loader2 className="h-7 w-7 animate-spin" /></div>;
        ...
        <section className="relative flex min-h-[55vh] items-center justify-center bg-black">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => navigator.share?.({ title: stream.title, url: window.location.href }).catch(() => undefined) || navigator.clipboard.writeText(window.location.href).then(() => toast.success("Link copied"))} className="grid h-11 w-11 place-items-center rounded-full bg-black/55 backdrop-blur-md"><Share2 className="h-5 w-5" /></button>
            </div>
          </div>
        </section>
