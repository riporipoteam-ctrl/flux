...
export default function GamesPage() {
  ...
  const filteredGames = useMemo(() => {
    ...
    return BROWSER_GAMES.filter((game) => {
      ...
      const queryMatch =
        !normalized ||
        [game.title, game.author, game.shortDescription, game.description, ...game.categories]
          .join(" ")
          .toLowerCase()
          .includes(normalized);
  ...
  const toggleFavorite = (slug: string) => {
    setFavorites((current) => {
      ...
      try {
        ...
        // Keep the current-session UI working when storage is blocked.
      }
      return next;
    });
  };
  ...
  const randomGame = () => {
    ...
    window.location.assign(hrefForGame(picked));
  };
  ...
  return (
    <div className="min-h-screen pb-14">
      <header className="sticky top-0 z-40 hidden border-b border-border/70 bg-background/88 px-5 py-3 backdrop-blur-xl lg:block">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Playable inside Flux on mobile, tablet and PC.</p>
          </div>
      ...
      <main className="px-3 pt-3 sm:px-5 sm:pt-5">
        <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-black shadow-[0_22px_70px_rgba(0,0,0,.28)] sm:rounded-[34px]">
          <div className="relative aspect-[16/13] min-h-[440px] sm:aspect-[16/8] sm:min-h-[500px]">
            <GameCoverArt game={hero} />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,.94)_0%,rgba(0,0,0,.72)_44%,rgba(0,0,0,.08)_82%)]" />
            <motion.div
            ...
            >
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/55">Featured this week</p>
              <h2 className="mt-3 text-4xl font-black leading-[0.92] tracking-[-0.06em] text-white sm:text-6xl">
                {hero.title}
              </h2>
              <p className="mt-4 max-w-xl text-sm leading-6 text-white/68 sm:text-base">{hero.description}</p>
              <div className="mt-5 flex flex-wrap gap-2">
                {hero.categories.map((item) => (
                  <span key={item} className="rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-[10px] font-bold text-white/75 backdrop-blur-md">
                    {item}
                  </span>
                ))}
              </div>
              <div className="mt-7 flex flex-wrap gap-3">
                <Link
                  href={hrefForGame(hero)}
                  className="inline-flex h-12 items-center gap-2 rounded-full bg-white px-6 text-sm font-black text-black transition hover:-translate-y-0.5"
                >
                ...
                <button
                  type="button"
                  onClick={randomGame}
                  className="inline-flex h-12 items-center gap-2 rounded-full border border-white/20 bg-black/30 px-5 text-sm font-black text-white backdrop-blur-md transition hover:bg-white/10"
                >
                  <Dices className="h-4.5 w-4.5" /> Surprise me
                </button>
              </div>
            </motion.div>
          </div>
        ...
        <section className="mt-8 border-y border-border/70 py-4">
          <div className="relative">
            ...
            <input
              ...
              className="h-12 w-full rounded-2xl border border-border bg-card pl-11 pr-12 text-sm font-semibold outline-none transition focus:border-foreground/25 focus:ring-4 focus:ring-foreground/5"
            />
            {query ? (
              <button
                type="button"
        ...
        <section className="mt-10">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Library</p>
          ...
          {!filteredGames.length ? (
            <div className="mt-5 grid min-h-56 place-items-center rounded-[24px] border border-dashed border-border p-8 text-center">
              <div>
                <Search className="mx-auto h-7 w-7 text-muted-foreground" />
                <h3 className="mt-3 font-black">Nothing found</h3>
...
function GameCard({
  ...
  return (
    <motion.article
      ...
      transition={{ duration: reduceMotion ? 0 : 0.25, delay: Math.min(index * 0.025, 0.14) }}
      className="group overflow-hidden rounded-[22px] border border-border bg-card transition hover:border-foreground/20 hover:shadow-[0_18px_45px_rgba(0,0,0,.11)]"
    >
      <Link href={hrefForGame(game)} className="block">
        <div className={`relative overflow-hidden ${compact ? "aspect-[16/9]" : "aspect-[16/10]"}`}>
          ...
          <button
            ...
            className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full border border-white/15 bg-black/45 text-white backdrop-blur-md transition hover:bg-black/70"
          ...
          <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-3">
            <span className="rounded-full bg-black/55 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-white backdrop-blur-md">
              {game.categories[0]}
            </span>
            <span className="grid h-9 w-9 place-items-center rounded-full bg-white text-black transition group-hover:translate-x-0.5">
        ...
        <div className={compact ? "p-4" : "p-5"}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className={`${compact ? "text-base" : "text-xl"} truncate font-black tracking-[-0.035em]`}>
                {game.title}
              </h3>
            ...
            {!compact ? <span className="text-[10px] font-black text-emerald-600">FREE</span> : null}
          </div>
          {!compact ? <p className="mt-3 line-clamp-2 text-sm leading-6 text-muted-foreground">{game.shortDescription}</p> : null}
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-border/70 pt-3 text-[9px] font-black uppercase tracking-[0.1em] text-muted-foreground">
