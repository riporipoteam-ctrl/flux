...
export default function GiftsPage() {
  ...
  return (
    <XPage>
      <XHeader title="Gifts" subtitle={`${formatCount(profile?.coins || 0)} Coins`} icon={Gift} hideOnMobile />

      <section className="x-hero">
        <p className="mb-3 text-[11px] font-black uppercase tracking-[0.16em] text-[var(--v8-accent)]">Animated gifts</p>
        <h1>Send something that actually moves.</h1>
        <p>Every gift is a hand-built animated scene. Send one to a person, or attach it to a post, Story or Live.</p>
        <div className="x-hero-actions">
          <span className="inline-flex items-center gap-2 rounded-full border border-[var(--v8-line)] bg-[var(--v8-panel)] px-4 py-2 text-sm font-black">
            <Coins className="h-4 w-4 text-[var(--v8-orange)]" />{formatCount(profile?.coins || 0)} Coins
          </span>
          <Link href="/shop" className="x-btn x-btn-hollow"><ShoppingBag className="h-4 w-4" />Open shop</Link>
        </div>
      </section>

      <div className="mx-auto w-full px-3 py-6 sm:px-5 sm:py-9">
