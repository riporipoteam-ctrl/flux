...
export default function PremiumPage() {
  ...
  const purchase = async (tier: "basic" | "premium") => {
    if (!user) return;
    setBuying(tier);
    try {
      ...
      <section className="overflow-hidden border-b border-black/6 bg-[#0b0c10] px-4 py-12 text-white dark:border-white/10 sm:py-16">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl"><span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-400 text-black"><Crown className="h-6 w-6" /></span><h1 className="mt-6 text-4xl font-black leading-[.94] tracking-[-.065em] sm:text-6xl">Make Flux work harder for you.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-white/58">Use Flux Coins to unlock more AskAI, creator tools, animated identity features and premium rewards. No card checkout is required.</p></div>
            <div className="min-w-[280px] rounded-[26px] border border-white/10 bg-white/6 p-5 backdrop-blur-xl"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-400/15 text-amber-300"><Coins className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-white/35">Your balance</p><p className="mt-1 text-2xl font-black">{formatCount(profile?.coins || 0)} Coins</p></div></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-violet-400 transition-all" style={{ width: `${percent}%` }} /></div><div className="mt-2 flex justify-between text-[10px] text-white/38"><span>AskAI {profile?.askAIUsage?.used || 0}/{profile?.askAIUsage?.limit || 20}</span><span>{100 - percent}% left</span></div></div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-3 py-8 sm:px-5 sm:py-12">
        <div className="grid gap-4 lg:grid-cols-3">
          {plans.map((plan) => {
            ...
            return <article key={plan.id} className={`relative overflow-hidden rounded-[28px] border p-5 sm:p-7 ${highlighted ? "border-amber-400/40 bg-[#15120b] text-white shadow-[0_25px_80px_rgba(245,158,11,.16)]" : "border-black/7 bg-white dark:border-white/10 dark:bg-[#101114]"}`}>
              ...
              <div className="mt-7">{active ? <Button disabled className="h-12 w-full rounded-full bg-emerald-500 text-white"><BadgeCheck className="h-5 w-5" />Current plan</Button> : plan.id === "free" ? <Button variant="outline" disabled className="h-12 w-full rounded-full">Included</Button> : <Button onClick={() => void purchase(plan.id)} disabled={buying !== null} className={`h-12 w-full rounded-full font-black ${highlighted ? "bg-amber-400 text-black hover:bg-amber-300" : ""}`}>{buying === plan.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Coins className="h-4 w-4" />}Unlock {plan.name}</Button>}</div>
            </article>;
          })}
        </div>
