      ...
      <header className="relative z-20 hidden border-b border-border/70 px-5 py-3 lg:sticky lg:top-0 lg:z-30 lg:block glass-strong">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Posts from your people and the wider Flux community.</p>
          </div>
    ...
    return (
      <div className="space-y-2 px-2 py-2">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="flex gap-3 rounded-[22px] border border-border/70 bg-card p-4 shadow-soft">
  ...
  if (!posts.length) {
    return (
      <EmptyState
        ...
        action={<Button variant="outline" onClick={onRefresh}>Refresh</Button>}
      />
    );
  }
